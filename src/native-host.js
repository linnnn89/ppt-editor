...
import { processIdentity, waitForExit, terminateOwnedProcess } from './windows.js';

export class NativeHost {
  constructor(job, onState = async () => {}) { this.job = job; this.onState = onState; this.pending = new Map(); this.broken = false; }
  ...
  async start() {
    ...
    this.worker.on('message', message => {
      if (message.stage) { this.lastStage = message.stage; return; }
  ...
  async request(method, args = {}, timeoutMs = 60000) {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.worker.send({ id, method, args }, error => { if (error) { clearTimeout(timer); this.pending.delete(id); reject(error); } });
    });
  }
  ...
  async shutdown(interrupted = false) {
    ...
    let result;
    if (!this.exited && !this.broken) result = await this.request('shutdown', { interrupted }, 30000);
    const workerExited = await waitForExit(this.identity, 10000);
    let officeExited = null;
    const observedOffice = [];
    const observations = result?.lease?.observedNewProcesses || [];
    ...
    return { ...result, worker: this.identity, workerExited, officeExited, observedOffice, cleanup, ...(!result ? { outcome: 'outcome_unknown' } : {}) };
  }
}

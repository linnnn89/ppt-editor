...
import { hash, stableJson } from './storage.js';
import { readPackage, checkNativeSafe } from './ooxml.js';
...
function fontSnapshot(font) {
  ...
  return { fontSize: Number(font.Size) || null, fontFace: String(font.Name || '') || null, bold: tri(font.Bold), italic: tri(font.Italic), color, source: 'native-effective-properties' };
}
    ...
    function visit(shapes, groupPath = [], notes = false) {
      for (let i = 1; i <= Number(shapes.Count); i++) {
        ...
        try {
          ...
          const kind = notes ? 'notes' : hasTable ? 'table' : type === 6 ? 'group' : hasText ? 'text' : [11, 13].includes(type) ? 'image' : 'shape';
          ...
          item.capabilities = [ ...(hasText && !hasTable ? ['replace_text', 'set_style'] : []), ...(hasTable ? ['set_table_cell'] : []), ...(!notes && groupPath.length === 0 && kind !== 'group' ? ['set_geometry'] : []) ];
          item.fingerprint = hash(stableJson(item)); objects.push(item); page.objectCount++;
...
function locate(doc, target) {
  const slide = doc.pres.Slides.FindBySlideID(target.slideId);
  const references = [slide];
  let shapes;
  if (target.kind === 'notes') { const notes = slide.NotesPage; references.push(notes); shapes = notes.Shapes; }
  ...
  for (const id of target.groupPath) { const group = find(shapes, id); references.push(group); shapes = group.GroupItems; references.push(shapes); }
  const shape = find(shapes, target.shapeId); references.push(shape);
...
function setFont(font, style) {
  ...
  if (style.fontFace) font.Name = style.fontFace;
...
async function closeDocument(doc, { interrupted = false } = {}) {
  ...
  doc.pres.Saved = -1; doc.pres.Close(); release(doc.pres); doc.pres = null; documents.delete(doc.id);
}

async function openDocument(args) {
  const bytes = await fs.readFile(args.path); checkNativeSafe(await readPackage(bytes));
  ensureApplication();
  ...
  const doc = { id: args.documentId, pres, directory: args.directory, revision: args.revision || 0, generation: args.generation || 1, visible: args.visible, unusable: false };
...
async function apply(args) {
  ...
  try {
    ...
    const nextCheckpoint = await checkpoint(doc, path.join(doc.directory, `native-${doc.generation}-${doc.revision + 1}.pptx`));
...
async function shutdown(interrupted = false) {
  ...
  clearInterval(pump); releaseMutex?.(); releaseSta?.();
